...
type Repositories = { description: null | string, name: string }[]
...
async function main() {
	...
	const { projectType }: { projectType: "new" | "existing" }
	...
	async function promptNewRepoName() {
		const { repoName }: { repoName: string } = await inquirer.prompt([
			{
				type: "input",
				name: "repoName",
	...
	async function promptUseTemplate() {
		const { useTemplate }: { useTemplate: boolean }
			= await inquirer.prompt([
				{
					type: "list",
					name: "useTemplate",
	...
	async function promptSelectRepo(repos: Repositories) {
		const { selectedTemplateName }: { selectedTemplateName: string } = await inquirer.prompt([
			{
				type: "list",
	...
	async function fetchRepos(template = false) {
		...
		const repositoriesRaw = runAndGet(
			`gh api orgs/${ORG_NAME}/repos --paginate --jq '.[] | select(.is_template==${template}) | {name: .name, description: .description}'`
		)
		...
		const repositories: Repositories = repositoriesRaw
			.split("\n")
			.filter(Boolean)
	...
	function cloneRepo(repoName: string) {
